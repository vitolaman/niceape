...
/**
 * Get the pool config key(s) based on the current network configuration
 * @returns Array of pool config keys for the current network
 */
export function getPoolConfigKeys(): string[] {
  const network = process.env.NEXT_PUBLIC_NETWORK || 'mainnet';

  let poolConfigKey: string | undefined;
  if (network === 'devnet') {
    poolConfigKey = process.env.NEXT_PUBLIC_DEVNET_POOL_CONFIG_KEY;
  } else {
    poolConfigKey = process.env.NEXT_PUBLIC_MAINNET_POOL_CONFIG_KEY;
  }

  // Fallback to original env var if network-specific ones don't exist
  const fallbackKey = process.env.NEXT_PUBLIC_POOL_CONFIG_KEY;
  const finalKey = poolConfigKey || fallbackKey;
